...
const ProtectedRoute = {
    oninit: vnode => {
        ...
        if (!SecurityModel.isAuthenticated()) {
            ...
            console.log("[Audit] User role not loaded, waiting...");
            SecurityModel.loadUserRole().then(() => {
                if (SecurityModel.role === null) {
                    ...
                    console.log("[Audit] User does not have the required role, redirecting to unauthorized");
    ...
    onbeforeupdate: vnode => {
        console.log("[Audit] Checking authentication and role on route update...");
        if (!SecurityModel.isAuthenticated()) {
            console.log("[Audit] User not authenticated, redirecting to login");
            m.route.set('/login');
            return false;
